...
public class Table implements Iterable<Record>, Closeable {
  ...
  public RecordID addRecord(List<DataType> values) throws DatabaseException {
    ...
    byte[] encoding = this.schema.encode(record);
    int pageNumToReturn = 0;
    int slotNumToReturn = 0;

    if (this.freePages.isEmpty()) {
      int newPageNum = this.allocator.allocPage();
      Page newPage = this.allocator.fetchPage(newPageNum);
      newPage.writeBytes(this.pageHeaderSize, this.schema.getEntrySize(), encoding);
      byte b = 1;
      this.writeBitToHeader(newPage, 0, b);
      this.freePages.add(newPageNum);
      pageNumToReturn = newPageNum;
    } else {
      Page currPage = this.allocator.fetchPage(this.freePages.first());
      byte[] currHeader = this.readPageHeader(currPage);
      int size = currHeader.length;
      int slotNum = 0;
      for (int i = 0; i < size * 8; i++) {
        int byteOffset = slotNum / 8;
        int bitOffset = 7 - (slotNum % 8);
        byte targetByte = currHeader[byteOffset];
        if (((targetByte >> bitOffset) & 1) == 0) {
          int posToWrite = this.pageHeaderSize + (slotNum*encoding.length);
          currPage.writeBytes(posToWrite, this.schema.getEntrySize(), encoding);
          byte b = 1;
          this.writeBitToHeader(currPage, slotNum, b);
          pageNumToReturn = this.freePages.first();
          slotNumToReturn = slotNum;
        } else {
          slotNum++;
        }
      }
      if (!spaceOnPage(currPage)) {
        this.freePages.remove(this.freePages.first());
      }
    }
    this.stats.addRecord(record);
    this.numRecords++;
    return new RecordID(pageNumToReturn, slotNumToReturn);
  }
  ...
  public Record deleteRecord(RecordID rid) throws DatabaseException {
    if (this.checkRecordIDValidity(rid)) {
      ...
      int posToRead = this.pageHeaderSize + (slotNum * this.schema.getEntrySize());
      byte[] oldEncoding = currPage.readBytes(posToRead, this.schema.getEntrySize());
      Record oldRecord = this.schema.decode(oldEncoding);
      byte b = 0;
      this.writeBitToHeader(currPage, slotNum, b);
      if (numValidEntries(currPage) == 0) {
        this.freePages.remove(pageNum);
        this.allocator.freePage(pageNum);
      }
      this.stats.removeRecord(oldRecord);
      this.numRecords--;
      return oldRecord;
    } else {
      throw new DatabaseException("invalid rid");
    }
  }
  ...
  public Record getRecord(RecordID rid) throws DatabaseException {
    if (this.checkRecordIDValidity(rid)) {
      Page currPage = this.allocator.fetchPage(rid.getPageNum());
      byte[] code = currPage.readBytes(this.pageHeaderSize+(this.schema.getEntrySize()*rid.getSlotNumber()),this.schema.getEntrySize());
      return this.schema.decode(code);
    } else {
      throw new DatabaseException("invalid rid");
    }
  }
  ...
  public Record updateRecord(List<DataType> values, RecordID rid) throws DatabaseException {
    if (this.checkRecordIDValidity(rid)) {
      ...
      Page currPage = this.allocator.fetchPage(pageNum);
      int posToWrite = this.pageHeaderSize + (slotNum * encoding.length);
      byte[] oldEncoding = currPage.readBytes(posToWrite, this.schema.getEntrySize());
      currPage.writeBytes(posToWrite, this.schema.getEntrySize(), encoding);
      Record oldRecord = this.schema.decode(oldEncoding);
      this.stats.removeRecord(oldRecord);
      this.stats.addRecord(record);
  ...
  private boolean checkRecordIDValidity(RecordID rid) throws DatabaseException {
    ...
    try {
      pageToTest = this.allocator.fetchPage(rid.getPageNum());
    } catch (PageException e) {
      throw new DatabaseException(e);
    }
    byte[] header = this.readPageHeader(pageToTest);
    int slotNum = rid.getSlotNumber();
    int byteOffset = slotNum / 8;
    int bitOffset = 7 - (slotNum % 8);
  ...
  private void setEntryCounts() {
    int sizeOfRecord = this.schema.getEntrySize();
    int sizeOfEightPack = (sizeOfRecord * 8) + 1;
    int numOfEightPacks = Page.pageSize / sizeOfEightPack;
    this.pageHeaderSize = numOfEightPacks;
    this.numEntriesPerPage = numOfEightPacks * 8;
  }
...
//  My implementation which has bugs
//  private class TableIterator implements Iterator<Record> {
...
//          this.currPageNum = this.currPage.getPageNum();
//        }
//      }
//    }
//
...
//      if (this.currSlotNum+1 >= Table.this.numEntriesPerPage) {
//        this.currSlotNum = 0;
//        this.currPage = this.pageIterator.next();
//        this.currPageNum = this.currPage.getPageNum();
//      } else {
...
//          Record r;
//          try {
//            r = getRecord(new RecordID(this.currPageNum, this.currSlotNum));
//            this.recordNum++;
//            updateRID();
//            return r;
