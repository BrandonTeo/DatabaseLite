...
public class IntHistogram implements Histogram<Integer> {
    ...
    public float computeReductionFactor(PredicateOperator predicate,
        ...
        //Compute RF values using formulas in class
        switch (predicate) {
            case EQUALS:
                reductionFactor = 1/(float)dist;
                break;
            case LESS_THAN:
                reductionFactor = ((float)(val-min)/(max-min));
                break;
            case LESS_THAN_EQUALS:
                reductionFactor = ((1/(float)dist)+((float)(val-min)/(max-min)));
                break;
            case GREATER_THAN:
                reductionFactor = ((float)(max-val)/(max-min));
                break;
            case GREATER_THAN_EQUALS:
                reductionFactor = ((1/(float)dist)+((float)(max-val)/(max-min)));
                break;
            case NOT_EQUALS:
                reductionFactor = (1 - (1/(float)dist));
                break;
            default:
                break;
        }
