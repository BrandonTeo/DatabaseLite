...
import edu.berkeley.cs186.database.table.stats.IntHistogram;
...
public class IndexScanOperator extends QueryOperator {
    ...
    public int estimateIOCost() throws QueryPlanException {
        int numRecordsLeft=1;
        int numPagesIndex=1;
        float rf = 1;
        try {
            numRecordsLeft = (int) this.transaction.getNumRecords(this.tableName);
            numPagesIndex = this.transaction.getNumIndexPages(this.tableName,this.columnName);
            rf = this.transaction.getStats(this.tableName).getReductionFactor(this.columnIndex,this.predicate,this.value);
        } catch (DatabaseException e) {
        }

        return (int) Math.ceil((numRecordsLeft+numPagesIndex)*rf);
    }
