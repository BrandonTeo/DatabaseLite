...
public class BPlusTree {
   ...
   */
    ...
    public BPlusIterator(LeafNode leaf, DataType key, boolean scan) {
      ...
      this.isScan = scan;
      if (scan) {
        this.currLeafIter = leaf.scanFrom(this.lookupKey);
      } else {
        this.currLeafIter = leaf.scanForKey(this.lookupKey);
      }
    }
    ...
    public boolean hasNext() {
      if (this.currLeafIter.hasNext()) {
        ...
        if (this.currLeaf.getNextLeaf() != -1) {
          LeafNode nextLeaf = (LeafNode) BPlusNode.getBPlusNode(this.currLeaf.getTree(), this.currLeaf.getNextLeaf());
          Iterator<RecordID> nextLeafIter;
          if (this.lookupKey == null) {
            nextLeafIter = nextLeaf.scan();
          } else if (this.isScan) {
            nextLeafIter = nextLeaf.scanFrom(this.lookupKey);
          } else {
            nextLeafIter = nextLeaf.scanForKey(this.lookupKey);
          }
          if (nextLeafIter.hasNext()) {
            return true;
          }
        }
        return false;
