...
public class TestBPlusTree {
  ...
  public void testBPlusTreeSortedScanSimple() {
    for (int i = 0; i < 400; i++) {
      bp.insertKey(new IntDataType(0), new RecordID(i, i));
    }
    Iterator<RecordID> iter = bp.sortedScan();
    assertTrue(iter.hasNext());

    //Simple test to scan all elements
    for (int i = 0; i < 400; i++) {
      assertTrue(iter.hasNext());
      iter.next();
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testBPlusTreeSortedScanFromSimple() {
    for (int i = 0; i < 400; i++) {
      bp.insertKey(new IntDataType(i), new RecordID(i, i));
    }
    Iterator<RecordID> iter = bp.sortedScanFrom(new IntDataType(100));
    assertTrue(iter.hasNext());

    //Only 300 entries have key bigger than 100
    for (int i = 0; i < 300; i++) {
      assertTrue(iter.hasNext());
      iter.next();
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testBPlusTreeLookupKeySimple() {
    for (int i = 0; i < 400; i++) {
      if (i % 5 == 0) {
        bp.insertKey(new IntDataType(0), new RecordID(i,i));
      } else {
    ...
    Iterator<RecordID> iter = bp.lookupKey(new IntDataType(0));
    assertTrue(iter.hasNext());

    //We will should 80 zeros in this iterator
    for (int i = 0; i < 80; i++) {
      assertTrue(iter.hasNext());
      iter.next();
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testBPlusTreeSortedScanReverse() {
    for (int i = 0; i < 400; i++) {
      bp.insertKey(new IntDataType(399-i), new RecordID(i, i));
    }
    Iterator<RecordID> iter = bp.sortedScan();
    assertTrue(iter.hasNext());

    //Inserting in any order should have no effect on sorted scan
    for (int i = 0; i < 400; i++) {
      assertTrue(iter.hasNext());
      iter.next();
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testBPlusTreeLookupKeyNoSuchElement() {
    for (int i = 1; i < 401; i++) {
      bp.insertKey(new IntDataType(i), new RecordID(i, i));
    }
    Iterator<RecordID> iter = bp.lookupKey(new IntDataType(0));

    //This iterator should not be able to iterate through anything
    assertFalse(iter.hasNext());
  }

  @Test
  @Category(StudentTest.class)
  public void testBPlusTreeSortedScanFromLargerValues() {
    for (int i = 0; i < 400; i++) {
      bp.insertKey(new IntDataType(i), new RecordID(i, i));
    }
    Iterator<RecordID> iter = bp.sortedScanFrom(new IntDataType(401));
