...
public class TestTable {
  ...
  @Test
  @Category(StudentTest.class)
  public void testTableIteratorSimple() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    RecordID[] recordIds = new RecordID[500];
    for (int i = 0; i < 500; i++) {
      input.getValues().get(1).setInt(i);
      recordIds[i] = table.addRecord(input.getValues());
    }

    //Simple test of number or elements iterator iterates through
  ...
  @Test
  @Category(StudentTest.class)
  public void testTableIteratorAfterDeletes() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    RecordID[] recordIds = new RecordID[500];
    for (int i = 0; i < 500; i++) {
      input.getValues().get(1).setInt(i);
    ...
    //Should iterate through less elements after some deletes
    Iterator<Record> iter = table.iterator();
    for (int i = 0; i < 300; i++) {
      assertTrue(iter.hasNext());
      iter.next();
  ...
  @Test
  @Category(StudentTest.class)
  public void testTableIteratorInMidstDeletes() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    RecordID[] recordIds = new RecordID[500];
    ...
    //Dynamic iterator should change values after in-between deletions
    for (int i=0; i < 200; i++) {
      table.deleteRecord(recordIds[i+300]);
    }
    assertFalse(iter.hasNext());
  }
  ...
  @Test
  @Category(StudentTest.class)
  public void testTableIteratorValues() throws DatabaseException {
    RecordID[] recordIds = new RecordID[500];
    Record[] records = new Record[500];
    for (int i = 0; i < 500; i++) {
      Record input = TestUtils.createRecordWithAllTypes();
    ...
    //Tests that iter returns the correct set of records
    Iterator<Record> iter = table.iterator();
    for (int i=0; i < 500; i++) {
      assertTrue(iter.hasNext());
