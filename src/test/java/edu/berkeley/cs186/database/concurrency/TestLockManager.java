...
public class TestLockManager {
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest1() throws InterruptedException{
        //This test tests a simple shared-shared sequence
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should have shared lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should not have have shared lock yet

        thread2.start();
        thread2.join(100); //waits for thread to finish (timeout of .1 sec)
        assertFalse("Transaction 2 Thread should have finished", thread2.isAlive()); //T2 should not be blocked
        assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should now have the shared lock

        lockMan.releaseLock("A", 1); //Transaction 1 releasing its lock

        assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should not have shared lock
        assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should still have shared lock

        lockMan.releaseLock("A", 2); //Transaction 2 releasing its lock

        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should now not have shared lock
    }

    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest2() throws InterruptedException{
        //This test tests a simple exclusive-exclusive sequence
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
        ...
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should have exclusive lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should not have exclusive lock yet

        thread2.start();
        thread2.join(100); //waits for thread to finish (timeout of .1 sec)
        assertTrue("Transaction 2 Thread should have finished", thread2.isAlive()); //T2 should still be waiting
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should not have exclusive lock yet
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest3() throws InterruptedException{
        //This test tests a triple exclusive sequence
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
        ...
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should have exclusive lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should not have exclusive lock yet
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should not have exclusive lock yet
        ...
        thread3.join(100);
        assertTrue(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should now not have exclusive lock
    }

    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest4() throws InterruptedException{
        //This test tests that shared locks can coexist and all of them have to be released for an exclusive lock to be gained
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should have shared lock
        assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should have shared lock
        assertTrue(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T3 should have shared lock
        assertFalse(lockMan.holdsLock("A", 4, LockManager.LockType.EXCLUSIVE)); //T4 should not have exclusive lock
        ...
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 4, LockManager.LockType.EXCLUSIVE)); //T4 should now have exclusive lock
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest5() throws InterruptedException{
        //This test tests that a shared lock owned by one transaction can be upgraded to exclusive
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        thread2.start();
        thread2.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should have upgraded to exclusive lock
        assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED));
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest6() throws InterruptedException{
        //This test tests that if we have multiple shared locks, to upgrade one of them to exclusive lock,
        //we need to release the other shared locks until the one we want to upgrade is the only one left
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        thread4.start();
        thread4.join(100);
        assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should not have been able to upgrade to exclusive lock
        ...
        lockMan.releaseLock("A", 3);
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should now have been able to upgrade to exclusive lock
    }


    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest7() throws InterruptedException{
        //This test tests that if we have multiple shared locks, to upgrade one of them to exclusive lock,
        //we need to release the other shared locks until the one we want to upgrade is the only one left and
        //also that upgrades are prioritized in front of gaining an exclusive lock
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should have shared lock
        assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should have shared lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should not have exclusive lock
        assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should not have exclusive lock
        ...
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should now have been able to upgrade to exclusive lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T2 should not have shared lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should not have exclusive lock since upgrade is prioritized

        lockMan.releaseLock("A", 1);
        thread3.join(100);
        assertTrue(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should now have exclusive lock
    }


    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest8() throws InterruptedException{
        //This test tests that upgrades are prioritized
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        thread3.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should have shared lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should not have exclusive lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should not have exclusive lock
        assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should not have exclusive lock

        thread4.start();
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE)); //T1 should have upgraded to exclusive lock first
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should not have exclusive lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE)); //T3 should not have exclusive lock
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest9() throws InterruptedException{
        //This test tests that shared locks cannot be granted if there is an exclusive lock request in front of it in the queue
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
        ...
        thread1.join(100);
        assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.SHARED)); //T1 should have shared lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T2 should not have shared lock
        assertFalse(lockMan.holdsLock("A", 4, LockManager.LockType.SHARED)); //T3 should not have shared lock
        assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T1 should not have exclusive lock
        ...
        thread3.start();
        thread3.join(100);
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T3 should not have shared lock since there is an exclusive lock request in front of it on the queue
        thread4.start();
        thread4.join(100);
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T4 should not have shared lock since there is an exclusive lock request in front of it on the queue
        ...
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE)); //T2 should have exclusive lock
        assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T3 should not have shared lock
        assertFalse(lockMan.holdsLock("A", 4, LockManager.LockType.SHARED)); //T4 should not have shared lock


        lockMan.releaseLock("A", 2);
        thread3.join(100);
        thread4.join(100);
        assertTrue(lockMan.holdsLock("A", 3, LockManager.LockType.SHARED)); //T3 should now have shared lock
        assertTrue(lockMan.holdsLock("A", 4, LockManager.LockType.SHARED)); //T4 should now have shared lock
    ...
    @Test
    @Category(StudentTestP3.class)
    public void myLockingSequenceTest0() throws InterruptedException{
        //This test tests that we cannot downgrade locks
        final LockManager lockMan = new LockManager();
        Thread thread1 = new Thread(new Runnable() {
            public void run() {
                lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
